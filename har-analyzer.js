function analyze(har) {
  // track the entities based on the guid prefixes
  let guidEntityMap = {};
  /**
  ...
  const mxReqLog = requests.map((req, index) => {
    ...
    if (refreshInClient) {
      ...
      catalogGuidEntity(
        objectIds,
        refreshInClient.args.classnames,
        guidEntityMap
      );
    }
    ret.response = {
  ...
  for (const key in guidEntityMap) {
    const resultingRequests = mxReqLog.filter(
      (req) => !!req.analysis.params.find((p) => p.entityType === key)
    ).length;
